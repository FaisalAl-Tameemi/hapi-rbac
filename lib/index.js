...
defaults.options = {
    onError: (request, h, err) => {
        throw h.error(Boom.wrap(err, 401))
    },
    responseCode: {
        onDeny: 401,
        onUndetermined: 401
            ...
            return internals.retrievePolicy(config, request, (err, policy) => {
                ...
                RbacCore.evaluatePolicy(policy, wrappedDataRetriever, (err, result) => {
                    ...
                    if (result === RbacCore.DENY) {
                        return options.onError(request, h, Boom.create(options.responseCode.onDeny, 'No permissions to access this resource'))
                    }

                    if (result === RbacCore.UNDETERMINED) {
                        return options.onError(request, h, Boom.create(options.responseCode.onUndetermined, 'Could not evaluate access rights to resource'))
                    }
