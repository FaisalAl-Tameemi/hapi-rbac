...
internals.retrievePolicy = (config, request, callback) => {
    return new Promise((resolve, reject) => {
        return config instanceof Function ? config() : resolve(null)
    })
}
...
const register = (server, options) => {
    ...
    server.ext('onPostAuth', (request, h) => {
        ...
        console.log('@RBAC', request.auth.credentials)

        if (config && config !== CONFIG_NONE) {

            return internals
                .retrievePolicy(config, request)
                .then((policy) => {
                    console.log('@RBAC', policy)
                    ...
                    const evaluatePolicy = promisify(RbacCore.evaluatePolicy(policy, wrappedDataRetriever))
